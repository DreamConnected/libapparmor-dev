...
B<COMMA RULES> = ( I<CAPABILITY RULE> | I<NETWORK RULE> | I<MOUNT RULE> | I<PIVOT ROOT RULE> | I<UNIX RULE> | I<FILE RULE> | I<LINK RULE> | I<CHANGE_PROFILE RULE> | I<RLIMIT RULE> | I<DBUS RULE> | I<MQUEUE RULE> | I<MODULE RULE> )
...
B<MODULE_RULE> = [ I<QUALIFIERS> ] 'module' [ I<MODULE ACCESS PERMISSION> ] [ I<MODULE TARGET> ]

B<MODULE ACCESS PERMISSION> = ( 'load_data' | 'load_file' | 'request' )

B<MODULE TARGET> = ( '"' I<AARE> '"' | I<AARE> )
...
AppArmor supports module mediation. The module rule restricts loading
Linux kernel modules. The mediation is done by three different methods:
loading a module image, loading a module file, and requesting a module -
'load_data', 'load_file', and 'request', respectively.

Since only a binary image loaded into memory is required when using
'load_data', then no target needs to be specified. In turn, 'load_file'
and 'request' do require a target to be specified. 'load_file' needs
the specification of the file path, and 'request' needs the module name,
as it will be requested by the kernel to modprobe(8).

eg.

 module load_data,		#allow loading module image into kernel
 module load_file /path/foo,	#allow loading /path/foo into kernel
 module request foo,		#allow request of foo
