...
but are available to be used in policy.
  ...
  @{attach_path}  - the profile exec attachment path - if ones has been defined
...
the rofile attaches to an executable. If the kernel supports it as a
kernel variable, it will be set to the specific path that match the
executable at run time. If the kernel does not support kernel variables
it will have the same value as B<@{exec_path}>.
